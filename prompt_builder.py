...
- Review every entry in `stats.files_over_200` for structural maintainability, but never treat line count alone as a defect. For files from 200 to 299 lines, lower a structure/readability/widget-decomposition score only when the path plus `class_count`, `build_method_count`, or source excerpt provides concrete evidence of multiple unrelated responsibilities, excessive widget aggregation, or poor cohesion. A cohesive Bloc, Cubit, repository, model, constants file, or single-state-owner page may legitimately exceed 200 lines and must not be penalized merely for its size.
- Use graduated size evidence: 200-299 lines means "review cohesion"; 300-499 lines is a stronger maintainability signal when paired with multiple responsibilities; 500+ lines deserves explicit feedback, but still requires evidence about what should be separated. Prefer behavior-preserving structural suggestions (extract route groups, presentation sections, or feature-specific constants) and do not recommend changing business logic solely to reduce line count.
- When suggesting a split, name the exact responsibilities or widgets to extract and keep existing routes, callbacks, state ownership, API contracts, Bloc/Cubit transitions, and business behavior unchanged. Do not propose arbitrary one-class-per-file fragmentation or a generic refactor based only on a numeric threshold.
